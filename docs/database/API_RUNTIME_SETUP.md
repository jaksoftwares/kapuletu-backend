...
You can test the ingestion endpoint using the following command:

```powershell
Invoke-RestMethod -Uri "http://127.0.0.1:8000/ingestion" -Method Post -Body @{
    Body = "KES 1000 from John Doe for Welfare"
    From = "+254700000000"
} -ContentType "application/x-www-form-urlencoded"
```
*(Note: Ensure you have seeded the database first so the phone number is recognized as a registered treasurer).*
